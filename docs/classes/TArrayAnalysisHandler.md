...
***This class handles advanced data analysis for arrays.***
...
$objArray = new TArrayAnalysisHandler($arrayData); // Initialize TArrayAnalysisHandler class
echo $objArray->findMax(); // Finds the maximum value in the array.
...
* `float autocorrelation(int $lag)` : Calculates the autocorrelation of the array.
* `array bootstrapResampling(int $numSamples)` : Generates bootstrap samples from the array. 
* `array categorizeByThreshold(float $threshold)` : Categorizes the array based on a threshold.
* `float correlationCoefficient(array $otherArray)` : Calculates the Pearson correlation coefficient between two arrays.
* `float covariance(array $otherArray)` : Calculates the covariance between the internal array and another array.
* `float entropy()` : Calculates the entropy of the array.
* `array exponentialMovingAverage(float $alpha)` : Calculates the exponential moving average of the array.
* `mixed findMax()` : Finds the maximum value in the array.
* `mixed findMin()` : Finds the minimum value in the array.
* `array frequencyDistribution()` : Calculates the frequency distribution of the array.
* `array generateStatisticsReport()` : Automatically generates a report of basic statistical measures for the array.
* `float geometricMean()` : Calculates the geometric mean of the array.
* `array groupBy(callable $callback)` : Groups the array elements based on a callback function.
* `float harmonicMean()` : Calculates the harmonic mean of the array.
* `float interquartileRange()` : Calculates the interquartile range of the array.
* `float kurtosis()` : Calculates the kurtosis of the array.
* `float mean()` : Calculates the mean (average) of the array.
* `float median()` : Calculates the median of the array.
* `mixed mode()` :  Calculates the mode of the array.
* `array movingAverage(int $windowSize)` : Calculates the moving average of the array.
* `float percentile(float $percentile)` : Calculates the percentile of the array.
* `float range()` : Calculates the range of the array.
* `float skewness()` : Calculates the skewness of the array.
* `float standardDeviation()` : Calculates the standard deviation of the array.
* `float sumOfSquares()` : Calculates the sum of squares of the array elements.
* `float variance()` : Calculates the variance of the array.
* `float weightedAverage(array $weights)` : Calculates the weighted average of the array.
* `array zScoreNormalization()` : Normalizes the array using Z-score normalization.
